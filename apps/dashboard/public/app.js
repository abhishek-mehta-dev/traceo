...
const { looksLikeJson, parseJsonPreview, expandJsonValue } = window.TraceoJsonPreview;

function captureWasTruncated(value) {
  if (!value || typeof value !== 'object') return false;
  if (value.bodyTruncated === true) return true;
  const payload = value.payload || {};
  return Boolean(payload.request?.bodyTruncated || payload.response?.bodyTruncated);
}
...
function jsonTree(value, path, depth, key) {
  ...
  if (value !== null && typeof value === 'object') {
    ...
    const foldAttrs = ' class="j-line j-fold" role="button" tabindex="0" data-json-path="' + escapeHtml(path) + '" data-json-depth="' + depth + '"';
    const toggle = '<span class="j-toggle" aria-hidden="true">' + (open ? '▾' : '▸') + '</span>';

    if (!open) {
      return '<div' + foldAttrs + '>' + toggle + label +
    ...
    return '<div class="j-block">' +
      '<div' + foldAttrs + '>' + toggle + label + '<span class="jpunct">' + openBrace + '</span></div>' +
...
function jsonPanel(value, treeId) {
  ...
  const truncated = captureWasTruncated(value) || acc.recovered || state.jsonRecovered;
  ...
  return '<div class="json-panel">' +
    '<div class="json-tools">' +
      ...
      '<button type="button" class="' + (state.jsonMode === 'raw' ? 'active' : '') + '" data-json-mode="raw">Pretty</button>' +
      '<button type="button" class="ghost" data-json-copy="' + id + '">Copy</button>' +
    '</div>' +
    (truncated ? '<p class="json-note">Captured body was truncated. This is a formatted preview of the stored slice.</p>' : '') +
    (state.jsonMode === 'raw'
      ? '<pre class="code">' + highlightJson(parsed) + '</pre>'
      : '<div class="json-tree">' + jsonTree(parsed, treeId || 'json', 0) + '</div>') +
    '</div>';
}
...
function closeModal() {
  state.modal = null;
  renderModal();
  loadRequests().catch((err) => { els.status.textContent = err.message; });
}
...
const TRACEO_BASE = typeof window !== 'undefined' && typeof window.__TRACEO_BASE__ === 'string'
...
async function clearAllRequests() {
  ...
  if (!window.confirm('Clear every captured hop and event from Traceo? This cannot be undone.')) {
    return;
  }
  setClearing(true);
  try {
...
function renderDetail() {
  ...
  if (!request) {
    ...
    if (state.requests.length && (state.section === 'errors' || state.range) && !state.selected) {
      els.detail.innerHTML = '<div class="empty"><div class="empty-visual" aria-hidden="true"></div><h2>Hidden by filters</h2><p>The selected hop is still in the stream, but not in this view.</p><button class="primary" data-reset="1" type="button">Show all hops</button></div>';
    } else if (state.selected && state.filteredCount > 0) {
      els.detail.innerHTML = '<div class="empty"><div class="empty-visual" aria-hidden="true"></div><h2>Hop is on another page</h2><p>This hop is still selected. Use Prev and Next to move through the stream, or reset filters to return to the latest hops.</p></div>';
    } else {
      els.detail.innerHTML = '<div class="empty"><div class="empty-visual" aria-hidden="true"></div><h2>Nothing selected</h2><p>Pick a hop to inspect the request, response, and trace rail.</p></div>';
    }
    return;
  }
  const ctx = contextFrom(state.timeline);
...
async function loadRequests(options = {}) {
  ...
  } else if (state.live && wasFollowing && !state.modal && state.page === 1 && incoming[0] && incoming[0].requestId !== state.selected) {
    state.selected = incoming[0].requestId;
    state.selectedSummary = incoming[0];
...
function typing() {
  ...
  return el === els.search || tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT' || Boolean(el?.closest?.('[data-json-path]'));
}
...
els.detail.addEventListener('click', async (event) => {
  ...
  if (toggle) {
    toggleJsonPath(toggle.dataset.jsonPath, Number(toggle.dataset.jsonDepth || 0));
    if (state.modal) renderModal();
    else renderDetail();
    return;
  }
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Enter' || event.key === ' ') {
    ...
    if (fold) {
      event.preventDefault();
      toggleJsonPath(fold.dataset.jsonPath, Number(fold.dataset.jsonDepth || 0));
      if (state.modal) renderModal();
      else renderDetail();
      return;
    }
